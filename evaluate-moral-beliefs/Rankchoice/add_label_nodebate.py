...
def add_label(file_oo, file_new):
    ...
    with open(file_oo, 'r', encoding="gbk") as f:
        reader = csv.reader(f)
        for row in reader:
            zh_choice_before, zh_score_before = deal_chosco(row[5])
            # zh_choice_after, zh_score_after = deal_chosco(row[7])
            ni_choice_before, ni_score_before = deal_chosco(row[7])
            # ni_choice_after, ni_score_after = deal_chosco(row[11])
            

            new_row = [row[0], row[1], row[2], row[3], zh_choice_before, zh_score_before,  ni_choice_before, ni_score_before]
...
if __name__ == '__main__':
    file_oo = "D:\VScode\lxl\Rankchoice\FF_data\\nodebatedata\dilemma_gpt35_nodebate_answer.csv"
    file_new = "D:\VScode\lxl\Rankchoice\FF_data\\nodebatedata\gpt35_nodebate_ans.csv"
