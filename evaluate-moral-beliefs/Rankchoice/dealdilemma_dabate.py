...
def rowsort_onechoice(option, score, row, flag):
    ...
    if row[12] != '':
        a_principle.append(row[12])  #A对应的支持的道德原则
    if row[13] != '':
        a_principle.append(row[13])
    if row[14] != '':
        b_principle.append(row[14])
    if row[15] != '':
        b_principle.append(row[15])
...
def extract_option_score_from_csv(file_name):
    ...
    with open(file_name, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            # print("row:", row)
            choice1_before = row[6]  #正序辩论前的选项和分数
            choice1_after = row[7]  #正序辩论后的选项和分数
            choice2_before = row[10]  #逆序辩论前的选项和分数
            choice2_after = row[11]  #逆序辩论后的选项和分数

            option1_before, score1_before = extract_option_score(choice1_before)
            option1_after, score1_after = extract_option_score(choice1_after)
            option2_before, score2_before = extract_option_score(choice2_before)
            option2_after, score2_after = extract_option_score(choice2_after)
            ...
            if int(score1_after) > 1:
                rowsortpairs.extend(rowsort_onechoice(option1_after, score1_after, row, 1))
            if int(score2_before) > 1:
                rowsortpairs.extend(rowsort_onechoice(option2_before, score2_before, row, -1))
            if int(score2_after) > 1:
                rowsortpairs.extend(rowsort_onechoice(option2_after, score2_after, row, -1))
...
# list_test =  [('责任', '经济效益', '2'), ('可持续发展', '经济效益', '2')]
# print(deal_pairs(list_test))
...
file_moraldilemma = './FF_data/debatedata/dilemma_debate_gpttoer_ans.csv'##问题文件
file_results = './results_sort/debate_results/rank_debate_gpttoer_ans.csv'
# text = "选项：B；；；坚定分数：2；；；；"
