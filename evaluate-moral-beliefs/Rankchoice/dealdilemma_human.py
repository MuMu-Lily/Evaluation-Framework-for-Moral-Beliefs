...
def rowsort_onechoice(option, score, row, flag):
    ...
    if row[6] != '':
        a_principle.append(row[6])  #A对应的支持的道德原则
    if row[7] != '':
        a_principle.append(row[7])
    if row[8] != '':
        b_principle.append(row[8])
    if row[9] != '':
        b_principle.append(row[9])
...
def extract_option_score_from_csv(file_name):
    ...
    with open(file_name, 'r', encoding='gbk') as f:
        ...
        for row in reader:
            ...
            option1_before, score1_before = row[4], row[5]  #正序的选项和分数            
            ...
            print(type(score1_before))
            if float(score1_before) > 1:
...
file_moraldilemma = './FF_data/human.csv'##问题文件
file_results = './results_sort/rank_human.csv'
# text = "选项：B；；；坚定分数：2；；；；"
