...
# 向线程池提交一个任务, 20和10会作为action_a/b()方法的参数
    future1 = pool.submit(no_debate_gpt.eval, topic0)
    time.sleep(2)
    future2 = pool.submit(no_debate_gpt.eval, topic1)
...
if __name__ == '__main__':
    ...
    # file_write = "D:\VScode\lxl\Debate\\results_withsex\woman\dilemma_gpt_woman.csv"#会把直接进行评估的结果（包含交换选项顺序的结果）写入这个文件
    ...
    # file_name = "D:\VScode\lxl\Rankchoice\data\dilemma_withsex\dilemma_woman.csv"
    file_write = "D:\VScode\lxl\Debate\\rebuttal\\consistency\\r3_gpt_moralchoice.csv" #会把直接进行评估的结果（包含交换选项顺序的结果）写入这个文件
    eval_model(file_name, file_write)