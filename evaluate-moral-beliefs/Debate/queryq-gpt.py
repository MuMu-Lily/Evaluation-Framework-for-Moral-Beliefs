...
# 向线程池提交一个任务, 20和10会作为action_a/b()方法的参数
    future1 = pool.submit(gpt35.getresponse, Q[0])
    time.sleep(2)
    future2 = pool.submit(gpt35.getresponse, Q[1])
    time.sleep(2)
    future3 = pool.submit(gpt35.getresponse, Q[2])
    time.sleep(2)
    future4 = pool.submit(gpt35.getresponse, Q[3])
    time.sleep(2)
    future5 = pool.submit(gpt35.getresponse, Q[4])
    time.sleep(2)
    future6 = pool.submit(gpt35.getresponse, Q[5])
...
writename = "D:\\VScode\\lxl\\Debate\\results\\word-ans-gpt35-left.csv"
